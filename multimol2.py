...
def read_molecule(reader: TextIO) -> list:
    ...
    while reading:
        ...
        if line.startswith('END'):
            ...
            num = int(parts[1])
            if num != cnt:
                print("Atom number out of order.")
                print(num, cnt)
...
def read_all_molecules(reader: TextIO) -> list:
    ...
    while reading:
        ...
        if molecule:  # None is treated as False in an if statement
            compound_name = molecule[0]
            molecule.pop(0)
            # print(f"C: {compound_name}")
            result[compound_name] = molecule
        else:
            reading = False
    return result
...
if __name__ == '__main__':
    ...
    molecule_file.close()
    print(molecules["AMMONIA"])
    print(molecules["METHANOL"])
